...
if __name__ == "__main__":
    ...
    translations = {
        "English": {
            ...
            "remaining_trades": "Remaining trades today",
            "max_volume": "Max Trade Volume (USDT)",
            "limit_reached": "Daily trading limit reached!",
            "volume_exceeded": "Trade volume exceeds limit!",
            "api_key": "Enter API Key",
            "api_secret": "Enter API Secret"
        },
        "Español": {
            "title": "🚀 Panel de Trading de Criptomonedas",
            "trading_limits": "Límites de Trading",
            "max_daily_trades": "Máximo de Operaciones Diarias",
            "remaining_trades": "Operaciones restantes hoy",
            "max_volume": "Volumen Máximo de Operación (USDT)",
            "limit_reached": "¡Límite diario alcanzado!",
            "volume_exceeded": "¡El volumen de operación excede el límite!",
            "api_key": "Ingrese la Clave API",
            "api_secret": "Ingrese el Secreto API"
        }
    }

    # Update text based on selected language
    if language == "Español":
        st.title(translations["Español"]["title"])
        st.markdown(f"### {translations['Español']['trading_limits']}")
        max_daily_trades = st.slider(translations["Español"]["max_daily_trades"], 1, 50, 10)
        
        # Inicializar daily_trades si no existe
        if 'daily_trades' not in st.session_state:
            st.session_state.daily_trades = 0
            
        remaining_trades = max_daily_trades - st.session_state.daily_trades
        st.write(f"{translations['Español']['remaining_trades']}: {remaining_trades}")
        max_trade_volume = st.number_input(translations["Español"]["max_volume"], min_value=0.0, value=1000.0)
        # Initialize XeggeX API connection
        try:
            xeggex_api_key = st.text_input(translations["Español"]["api_key"], type="password")
            xeggex_api_secret = st.text_input(translations["Español"]["api_secret"], type="password")
            
            if xeggex_api_key and xeggex_api_secret:
                # XeggeX specific API initialization would go here
                # Note: Since XeggeX is not part of ccxt, we'd need custom implementation
                st.success("Conectado exitosamente a XeggeX")
                
                # Get market data from XeggeX
                def get_xeggex_market_data():
                    try:
                        # Placeholder for XeggeX API calls
                        # This would need to be implemented according to XeggeX's API documentation
                        pass
                        
                    except Exception as e:
                        st.error(f"Error al obtener datos de XeggeX: {str(e)}")
                
                # Update price information
                if st.button("Actualizar Precios"):
                    with st.spinner("Actualizando datos del mercado..."):
                        market_data = get_xeggex_market_data()
                        
                # Create trading interface
                st.markdown("### Panel de Trading XeggeX")
                
                # Market selection
                trading_pair = st.selectbox(
                    "Par de Trading",
                    ["BTC/USDT", "ETH/USDT", "XRP/USDT"]  # Add actual pairs from XeggeX
                )
                ...
                with col1:
                    if st.button("Comprar", type="primary"):
                        if amount > 0:
                            st.info(f"Orden de compra enviada: {amount} {trading_pair}")
                            
                with col2:
                    if st.button("Vender", type="primary"):
                        if amount > 0:
                            st.info(f"Orden de venta enviada: {amount} {trading_pair}")
                            
        except Exception as e:
            st.error(f"Error en la conexión con XeggeX: {str(e)}")
            # Wallet Section
            st.markdown("### Billetera XeggeX")
            ...
            with col1:
                st.metric("USDT", "0.00")
            with col2:
                st.metric(trading_pair.split('/')[0], "0.00")
            
            # Deposit Section
            st.markdown("#### Depositar Fondos")
            deposit_currency = st.selectbox(
                "Seleccionar Moneda para Depositar",
                ["USDT", "BTC", "ETH", "XRP"]
            )
            
            # Generate/display deposit address
            if st.button("Generar Dirección de Depósito"):
                # Placeholder for actual API call to get deposit address
                deposit_address = "0x1234...abcd"  # Example address
                st.code(deposit_address)
                st.info("Envíe sus fondos a esta dirección. Los depósitos pueden tardar hasta 30 minutos en acreditarse.")
            
            # Withdrawal Section
            st.markdown("#### Retirar Fondos")
            
            withdraw_currency = st.selectbox(
                "Seleccionar Moneda para Retirar",
                ["USDT", "BTC", "ETH", "XRP"]
            )
            
            withdraw_address = st.text_input(
                "Dirección de Retiro",
            ...
            if st.button("Retirar Fondos"):
                if withdraw_amount > 0 and withdraw_address:
                    # Placeholder for actual withdrawal API call
                    st.warning("Confirme los detalles de retiro:")
                    st.write(f"Moneda: {withdraw_currency}")
                    st.write(f"Cantidad: {withdraw_amount}")
                    st.write(f"Dirección: {withdraw_address}")
                    
                    if st.button("Confirmar Retiro"):
                        st.success("Solicitud de retiro enviada")
                else:
                    st.error("Por favor ingrese una cantidad válida y una dirección de destino")
            # Popular Cryptocurrencies Trading Section
            st.markdown("#### Trading de Criptomonedas")
            ...
            if st.button("Ejecutar Operación"):
                if trade_amount > 0:
                    ...
                    st.warning(f"Confirme los detalles de la operación:")
                    st.write(f"Par: {selected_pair}")
                    st.write(f"Tipo: {trade_type}")
                    st.write(f"Cantidad: {trade_amount}")
                    
                    if st.button("Confirmar Operación"):
                        st.success(f"Orden de {trade_type.lower()} enviada exitosamente")
                else:
                    st.error("Por favor ingrese una cantidad válida para operar")
            # Cryptocurrency Price Updates and Charts Section
            st.markdown("#### Precios y Gráficos en Tiempo Real")
            
            # Initialize API connections (you'll need to add your API keys in a secure way)
            try:
                # Fetch data from multiple sources for redundancy and comparison
                
                # CoinGecko data
                cg = CoinGeckoAPI() # type: ignore
                ...
                for coin in top_coins:
                    ...
                    with col2:
                        st.write(f"Precio: ${coin['current_price']:,.2f}")
                    with col3:
                        price_change = coin['price_change_percentage_24h']
                        color = 'green' if price_change > 0 else 'red'
                        st.markdown(f"24h: <span style='color:{color}'>{price_change:.2f}%</span>", unsafe_allow_html=True)
                
                # Create price alert section
                st.markdown("#### Configurar Alertas de Precio")
                alert_coin = st.selectbox(
                    "Seleccionar Criptomoneda para Alerta",
                    [coin['symbol'].upper() for coin in top_coins]
                )
                ...
                if st.button("Establecer Alerta"):
                    st.success(f"Alerta configurada para {alert_coin} {alert_condition.lower()} ${alert_price}")
                
                # Display interactive chart
                st.markdown("#### Gráfico de Precios")
                selected_coin_chart = st.selectbox(
                    "Seleccionar Criptomoneda para Gráfico",
                    [coin['symbol'].upper() for coin in top_coins]
                )
                
                # Get historical data for selected coin
                selected_coin_data = next(coin for coin in top_coins if coin['symbol'].upper() == selected_coin_chart)
                
                # Create price chart using plotly
                fig = go.Figure()
                fig.add_trace(go.Line(
                    y=selected_coin_data['sparkline_in_7d']['price'],
                    x=[datetime.now() - timedelta(days=7-i/24) for i in range(168)],
                    name=f"{selected_coin_chart} Price"
                ))
                fig.update_layout(
                    title=f"{selected_coin_chart}/USD últimos 7 días",
                    xaxis_title="Fecha",
                ...
                st.error(f"Error al cargar datos de precio: {str(e)}")
                # Add download buttons for different platforms
                st.markdown("#### Descargar App")
                st.markdown("Descarga nuestra aplicación para tu dispositivo:")
                ...
                with col1:
                    if st.button("📱 Android"):
                        st.markdown("""
                            1. Descarga el APK desde [Google Play Store](https://play.google.com/store)
                ...
                with col2:
                    if st.button("📱 iOS"): 
                        st.markdown("""
                            1. Visita el [App Store](https://apps.apple.com)
                            2. Busca "Crypto Team Trading"
                            3. Descarga e instala la aplicación
                        """)
                
                with col3:
                    if st.button("💻 Windows"):
                        st.markdown("""
                            1. Descarga el instalador desde [Microsoft Store](https://apps.microsoft.com)
                            2. Ejecuta el archivo .exe
                            3. Sigue las instrucciones de instalación
                        """)
                
                st.info("Nota: Las aplicaciones móviles y de escritorio te permitirán acceder a todas las funcionalidades incluso sin conexión a internet.")
                # Set Streamlit theme colors and styling
                st.markdown("""
                    <style>
                ...
                # Update chart styling to match Xeggex
                fig.update_layout(
                    plot_bgcolor='#0e1117',
                ...
                # Create language selector in the sidebar
                ...
                translations = {
                    ...
                    "Español": {
                        "title": "Plataforma de Trading de Criptomonedas",
                        "balance": "Saldo",
                ...
                # Set Streamlit page configuration with blue and black theme
                st.markdown("""
                    <style>
                        .stApp {
                ...
                """, unsafe_allow_html=True)
                # Set commission rate and wallet for automatic transfers
                commission_rate = 0.001  # 0.1% commission
                commission_wallet = "TP5vLL3MnVcb5yZ9i7mBQe3q9BVCrfiFUd"

                # Calculate commission amount based on trade value
                def calculate_commission(trade_amount, price):
                    trade_value = float(trade_amount) * float(price)
                    commission = trade_value * commission_rate
                    return commission

                # Get current price for selected pair
                try:
                    ticker = exchange.fetch_ticker(selected_pair) # type: ignore
                    current_price = ticker['last']
                    
                    # Display commission info
                    st.info(f"A commission of 0.1% will be charged on trades and sent to platform wallet")
                    
                    # Calculate and display estimated commission
                    if 'trade_amount' in st.session_state:
                        est_commission = calculate_commission(st.session_state.trade_amount, current_price)
                        st.write(f"Estimated commission: {est_commission:.8f} USDT")
                        
                except Exception as e:
                    st.error(f"Error fetching price: {str(e)}")
                    # Basic Trading Course Section
                    st.markdown("### Curso Básico de Trading en XEggEx")
                    
                    st.markdown("""
                    #### Módulo 1: Introducción a XEggEx
                    - ¿Qué es XEggEx?
                    - Cómo crear una cuenta
                    - Verificación y seguridad
                    - Navegación por la plataforma
                    
                    #### Módulo 2: Conceptos Básicos de Trading
                    - Tipos de órdenes en XEggEx
                    - Pares de trading disponibles
                    - Lectura de gráficos básicos
                    - Volumen y liquidez
                    
                    #### Módulo 3: Gestión de Riesgos
                    - Establecer límites de pérdidas
                    - Diversificación en XEggEx
                    - Tamaño de posición adecuado
                    - Uso de stop loss
                    
                    #### Módulo 4: Estrategias Básicas
                    - Trading de tendencias en XEggEx
                    - Análisis técnico básico
                    - Identificación de patrones
                    - Momentos óptimos de entrada/salida
                    """)
                    
                    # Course progress tracking
                    course_progress = st.progress(0)
                    current_module = st.selectbox(
                        "Seleccionar Módulo",
                        ["Módulo 1", "Módulo 2", "Módulo 3", "Módulo 4"]
                    )
                    
                    # Update progress based on module selection
                    progress_map = {
                        "Módulo 1": 25,
                        "Módulo 2": 50,
                        "Módulo 3": 75,
                    ...
                    course_progress.progress(progress_map[current_module])
                    
                    # Quiz section
                    st.markdown("#### Quiz Rápido")
                    if st.button("Comenzar Quiz"):
                        st.write("¿Cuál es la característica principal de XEggEx?")
                        quiz_options = st.radio(
                            "",
                        ...
                        if st.button("Verificar Respuesta"):
                            if quiz_options == "Exchange centralizado":
                                st.success("¡Correcto! XEggEx es un exchange centralizado.")
                            else:
